...
class Navigator:
    def __init__(self):
        ...
        self.gpsDiff = [0, 0, 0]
        self.StartSweep_pub = rospy.Publisher('/StartSweep', std_msgs.msg.Int8, queue_size=1)
        self.ContinueSweep_pub = rospy.Publisher('/ContinueSweep', std_msgs.msg.Int8, queue_size=1)
        self.StopSweep = rospy.Subscriber('/StopSweep', std_msgs.msg.Int8, self.stopSweepCallback, queue_size=1)
        self.MineFound = rospy.Subscriber('/MineFound', std_msgs.msg.Int8, self.mineDetectedCallback, queue_size=1)
        self.continueMoving = False
        self.mineLocatedBool = False

    def mineDetecedCallback(self, data):
        mineLocatedBool = True

    def stopSeepCallback(self, data):
        if data == 1:
            self.continueMovign = True
    ...
    def makeGridFromAPI(self):
        ...
        for index in range(0, gridLength):
            ...
            for index2 in range(0, gridHeight):
                self.grid[index].append({"value": self.gridData.grid[index].array[index2].value,
                                         "gps": {"x": self.gridData.grid[index].array[index2].gps.x,
                                                 "y": self.gridData.grid[index].array[index2].gps.x}})
        locationSet = False
        while locationSet == False:
            gpsArr2 = deepcopy(self.gpsArr)
            self.gpsDiff[0] = math.sqrt(
                abs(gpsArr2[0][0] - gpsArr2[1][0]) ** 2 + abs(gpsArr2[0][1] - gpsArr2[1][1]) ** 2)
            self.gpsDiff[1] = math.sqrt(
                abs(gpsArr2[2][0] - gpsArr2[1][0]) ** 2 + abs(gpsArr2[2][1] - gpsArr2[1][1]) ** 2)
            self.gpsDiff[2] = math.sqrt(
                abs(gpsArr2[2][0] - gpsArr2[0][0]) ** 2 + abs(gpsArr2[2][1] - gpsArr2[0][1]) ** 2)
            ...
            if (max(self.gpsDiff[0], self.gpsDiff[1], self.gpsDiff[2]) < 0.000003):
                ...
                for gpsIndex in range(0, len(self.grid)):
                    for gpsIndex2 in range(0, len(self.grid[0])):
                        print(abs(abs(self.grid[gpsIndex][gpsIndex2]["gps"]["x"] - self.currentGPSLocation.x) + abs(
                            self.grid[gpsIndex][gpsIndex2]["gps"]["y"] - self.currentGPSLocation.y)))
                        if (abs(abs(self.grid[gpsIndex][gpsIndex2]["gps"]["x"] - self.currentGPSLocation.x) + abs(
                                self.grid[gpsIndex][gpsIndex2]["gps"][
                                    "y"] - self.currentGPSLocation.y)) < locDifference and
                                self.grid[gpsIndex][gpsIndex2]["value"] == 5):
                            locDifference = abs(
                                self.grid[gpsIndex][gpsIndex2]["gps"]["x"] - self.currentGPSLocation.x) + abs(
                                self.grid[gpsIndex][gpsIndex2]["gps"]["y"] - self.currentGPSLocation.y)
                            # print(locDifference)
                            closeLocation = (gpsIndex, gpsIndex2)
                            # self.currLocation = closeLocation
                locationSet = True
        for index in range(self.currLocation[0] - self.halfSizeOfRobot,
                           self.currLocation[0] + self.halfSizeOfRobot + 1):
            for index2 in range(self.currLocation[1] - self.halfSizeOfRobot,
                                self.currLocation[1] + self.halfSizeOfRobot + 1):
        ...
        for index in range(0, gridLength):
            ...
            for index2 in range(0, gridHeight):
                self.grid[index].append(
                    {"value": 5, "gps": {"x": self.initialPos[1] + 0.000002, "y": self.initialPos[0] + 0.000002}})
        for index in range(self.currLocation[0] - self.halfSizeOfRobot,
                           self.currLocation[0] + self.halfSizeOfRobot + 1):
            for index2 in range(self.currLocation[1] - self.halfSizeOfRobot,
                                self.currLocation[1] + self.halfSizeOfRobot + 1):
    ...
    def determinePath(self, currTarget, pathType):
        ...
        while targetRoute == []:
            ...
            for potentialSpacesIndex in potentialSpaces:
                ...
                leftRoute.append(4)
                down = {"x": potentialSpacesIndex["x"], "y": potentialSpacesIndex["y"] - 1,
                        "dist": potentialSpacesIndex["dist"] + 1, "route": downRoute, "target": 0}
                right = {"x": potentialSpacesIndex["x"] + 1, "y": potentialSpacesIndex["y"],
                         "dist": potentialSpacesIndex["dist"] + 1, "route": rightRoute, "target": 0}
                up = {"x": potentialSpacesIndex["x"], "y": potentialSpacesIndex["y"] + 1,
                      "dist": potentialSpacesIndex["dist"] + 1, "route": upRoute, "target": 0}
                left = {"x": potentialSpacesIndex["x"] - 1, "y": potentialSpacesIndex["y"],
                        "dist": potentialSpacesIndex["dist"] + 1, "route": leftRoute, "target": 0}
                ...
                for index in currentAdjacentSpaces:
                    ...
                    for archivedSpacesIndex in archivedSpaces:
                        if archivedSpacesIndex["x"] == index["x"] and archivedSpacesIndex["y"] == index["y"]:
                            inArchive = 1
                    if inArchive == 0:
                        if index["x"] - self.halfSizeOfRobot < 0 or index["y"] - self.halfSizeOfRobot < 0 or index[
                            "x"] + self.halfSizeOfRobot > len(self.grid) - 1 or index["y"] + self.halfSizeOfRobot > len(
                                self.grid[0]) - 1 or self.grid[index["x"]][index["y"]]["value"] == 4 or \
                                self.grid[index["x"]][index["y"]]["value"] == 3 or self.grid[index["x"]][index["y"]][
                            "value"] == 0 or self.grid[index["x"]][index["y"]]["value"] == 6:
                            index["dist"] = -1
                        elif self.grid[index["x"]][index["y"]]["value"] == 2:
                            match = 0
                            for index2 in newAdditions:
                                if index2["x"] == index["x"] and index2["y"] == index["y"] and index2["dist"] > index[
                                    "dist"]:
                                    index2["dist"] = index["dist"]
                                    index2["route"] = index["route"]
                                    match = 1
                                elif index2["x"] == index["x"] and index2["y"] == index["y"] and index2["dist"] <= \
                                        index["dist"]:
                                    match = 1
    ...
    def senseMine(self, sliderFirstReduced, sliderLastReduced):
        if self.currDirection == 1 and self.currLocation[1] - self.halfSizeOfRobot - 1 > 0:
            for index in range(self.currLocation[0] - self.halfSizeOfRobot + min(sliderFirstReduced, sliderLastReduced),
                               self.currLocation[0] - self.halfSizeOfRobot + max(sliderFirstReduced,
                                                                                 sliderLastReduced)):
        ...
        if self.currDirection == 2 and self.currLocation[0] + self.halfSizeOfRobot - +1 < len(self.grid):
            for index in range(self.currLocation[1] - self.halfSizeOfRobot + min(sliderFirstReduced, sliderLastReduced),
                               self.currLocation[1] - self.halfSizeOfRobot + max(sliderFirstReduced,
                                                                                 sliderLastReduced)):
    ...
    def bufferSingleSquare(self, location):
        for index in range(loacation[0] - self.halfSizeOfRobot, loaction[0] + self.halfSizeOfRobot + 1):
            for index2 in range(loacation[1] - self.halfSizeOfRobot, loaction[1] + self.halfSizeOfRobot + 1):
                if index > 0 and index2 > 0 and index < len(self.grid) and index2 < len(self.grid[0]) and (
                        self.grid[index][index2]["value"] == 2 or self.grid[index][index2]["value"] == 5):
    ...
    def establishBuffer(self):
        for index in range(0, len(self.grid)):
            for index2 in range(0, len(self.grid[0])):
                if index < self.halfSizeOfRobot or index2 < self.halfSizeOfRobot or index > len(
                        self.grid) - self.halfSizeOfRobot - 1 or index2 > len(self.grid[0]) - self.halfSizeOfRobot - 1:
                ...
                if self.grid[index][index2]["value"] == 0:
                    for index3 in range(index - self.halfSizeOfRobot, index + self.halfSizeOfRobot + 1):
                        for index4 in range(index2 - self.halfSizeOfRobot, index2 + self.halfSizeOfRobot + 1):
                            if index3 > 0 and index4 > 0 and index3 < len(self.grid) - 1 and index4 < len(
                                    self.grid[0]) - 1:
        ...
        while not rospy.is_shutdown():
            try:
                ...
                if (max(self.gpsDiff[0], self.gpsDiff[1], self.gpsDiff[2]) < 0.000003):
                    ...
                    for gpsIndex in range(0, len(self.grid)):
                        for gpsIndex2 in range(0, len(self.grid[0])):
                            if ((abs(self.grid[gpsIndex][gpsIndex2]["gps"]["x"] - abs(self.currentGPSLocation.x)) + abs(
                                    self.grid[gpsIndex][gpsIndex2]["gps"][
                                        "y"] - self.currentGPSLocation.y)) < locDifference):
                                locDifference = abs(
                                    self.grid[gpsIndex][gpsIndex2]["gps"]["x"] - self.currentGPSLocation.x) + abs(
                                    self.grid[gpsIndex][gpsIndex2]["gps"]["y"] - self.currentGPSLocation.y)
                                # print(locDifference)
                ...
                if mineDetectedBool:
                    if dist < 0.1 and keyboardP == False:
                        self.driveStraight(-0.2)
                    else:
                        mineDetectedBool = False
                        self.senseMine(self.halfSizeOfRobot, sliderPosition, sliderPosition)
                        targetPath, stop = self.determinePath((0, 0), self.halfSizeOfRobot, "Frontier Path")
                if case == "explore":
                    currTime = time.time()
                    ...
                    if targetPath != []:
                        if self.ContinueMoving:
                            ...
                            if self.currDirection == 2 and self.currLocation[0] + self.halfSizeOfRobot + 2 < len(
                                    self.grid):
                                print(self.currLocation, len(self.grid))
                                for index in range(self.currLocation[1] - self.halfSizeOfRobot,
                                                   self.currLocation[1] + self.halfSizeOfRobot + 1):
                            ...
                            if self.currDirection == 3 and self.currLocation[1] + self.halfSizeOfRobot + 2 < len(
                                    self.grid[0]):
                            ...
                            self.fullStop()
                        for index in range(self.currLocation[0] - self.halfSizeOfRobot,
                                           self.currLocation[0] + self.halfSizeOfRobot):
                            for index2 in range(self.currLocation[1] - self.halfSizeOfRobot,
                                                self.currLocation[1] + self.halfSizeOfRobot):
                                if index >= 0 and index2 >= 0 and index < len(self.grid) and index2 < len(
                                        self.grid[0]) and self.grid[index][index2]["value"] == 5:
                                    self.grid[index][index2]["value"] = 2
                    if stop:
                        self.fullStop()
                        # print(mines)
